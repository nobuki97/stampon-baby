...
export async function POST(req: NextRequest) {
  ...
  let photo: string, breed: string, color: string, pattern: string, feature: string, petName: string, style: string, phrases: string[], trial: boolean, sessionId: string, startIndex: number, masterBlobUrl: string
  try {
    ...
    if (!breed || !color) throw new Error("missing required fields")
      ...
      try {
        if (trial) {
          ...
          if (startIndex > 0 && masterBlobUrl) {
            ...
            masterB64 = await generateMaster(apiKey, photo, breed, color, pattern, feature, petName, style)
          ...
          for (let i = startIndex; i < 16; i += BATCH) {
            ...
            const results = await Promise.all(batch.map(idx => generateStamp(apiKey, breed, color, pattern, feature, petName, style, phrases[idx], idx, masterB64)))

...
async function generateMaster(
  ...
  formData.append(
    'prompt',
    `Transform this pet photo into a cute LINE sticker character. ` +
      `Character: ${characterDesc}. ` +
      `Style: ${stylePrompt}, large expressive eyes, full body, centered on white background, ` +
...
async function generateStamp(
  ...
  const res = await fetch(`${OPENAI_BASE}/images/generations`, {
    ...
    body: JSON.stringify({
      model: 'gpt-image-1',
      prompt:
        `LINE sticker of a cute ${characterDesc}. ` +
        `Pose: ${pose}. ` +
        `Japanese text "${phrase}" at the bottom in bold rounded white letters with dark outline. ` +
        `Style: ${stylePrompt}, large expressive eyes, white background, square format, ` +
...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey || apiKey === 'your_key_here') {
    return new Response(
      JSON.stringify({
        error: 'OpenAI API key が設定されていません。.env.local を確認してください。',
      }),
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        // Step 1: master character from the uploaded photo
        const masterB64 = await generateMaster(
          apiKey,
          photo,
          breed,
          color,
        ...
        // Steps 2–17: 16 stamps in order, using user-supplied phrases
        for (let i = 0; i < 16; i++) {
          const b64 = await generateStamp(
            apiKey,
            breed,
            color,
            pattern,
          ...
          send({ type: 'stamp', index: i, dataUrl: `data:image/png;base64,${b64}` })
        }

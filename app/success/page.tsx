...
export default function SuccessPage() {
  ...
  const startFlow = async (sid: string) => {
    ...
    try {
      ...
      if (!res.ok || !data.ok || !data.formData) {
        setErrorMessage(data.error ?? '支払いの確認に失敗しました')
        setPhase('error')
        return
      }
    ...
    if (!photo) {
      setErrorMessage('写真データが見つかりません。お手数ですが最初からやり直してください。\nご購入済みの場合はships.llc@gmail.comまでご連絡ください。')
      setPhase('error')
      return
    }
    ...
    if (!res.ok || !res.body) {
      ...
      setErrorMessage(msg.error ?? '生成に失敗しました')
      setPhase('error')
      return
    }
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          if (event.type === 'master') {
            ...
            sessionStorage.removeItem('stampon_photo')
            setPhase('done')
          }
        }
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-pink-50 via-white to-purple-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-2xl animate-bounce mb-3">💳</p>
          <p className="text-pink-500 font-bold animate-pulse">支払いを確認中...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-pink-50 via-white to-purple-50 flex items-center justify-center px-4">
        <div className="max-w-sm w-full text-center">
          ...
          <div className="bg-red-50 border border-red-200 rounded-2xl px-4 py-3 text-sm text-red-600 mb-6 whitespace-pre-line">
            {errorMessage}
          </div>
          <button onClick={() => router.push('/')}
            className="w-full py-3 rounded-full bg-gradient-to-r from-pink-400 to-purple-400 text-white font-bold shadow-md">
            トップに戻る
          </button>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto flex flex-col gap-6">
        {phase === 'generating' && (
          <div className="w-full bg-pink-100 rounded-full h-3 overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-pink-400 to-purple-400 transition-all duration-500"
              style={{ width: `${(completedCount / STAMP_COUNT) * 100}%` }} />
          </div>
        )}
        {masterDataURL && (
          <div className="flex flex-col items-center gap-2">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Master Character</p>
            <img src={masterDataURL} alt="マスターキャラクター" className="w-32 h-32 rounded-2xl object-cover border-4 border-pink-200 shadow-lg" />
          </div>
        )}
        <div className="grid grid-cols-4 gap-3 sm:gap-4">
          {Array.from({ length: STAMP_COUNT }, (_, i) => (
            <StampCard key={i} index={i} dataURL={stampDataURLs[i] ?? null} isGenerating={phase === 'generating' && i === stampDataURLs.length} />
          ))}
        </div>
